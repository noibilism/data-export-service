    ...
    def process_export(self, reference_id):
        ...
        try:
            ...
            if export.retry_count < current_app.config['MAX_RETRY_ATTEMPTS']:
                ...
                logger.error(
                    f"Export failed permanently after {export.retry_count} attempts: {reference_id}")
                return False
            ...
            with open(output_file_path, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                while True:
                    ...
                    if row_count % (chunk_size * 10) == 0:
                        logger.info(
                            f"Processed {row_count} rows for export: {table_name}")

        logger.info(
            f"Export completed: {row_count} rows written to {output_file_path}")
        return row_count
    ...
    def get_export_metrics(self):
        ...
        try:
            with db.session.begin():
                ...
                completed_jobs = db.session.query(Export).filter_by(
                    status=ExportStatus.COMPLETED).count()
                failed_jobs = db.session.query(Export).filter_by(
                    status=ExportStatus.FAILED).count()
                pending_jobs = db.session.query(Export).filter_by(
                    status=ExportStatus.PENDING).count()
                in_progress_jobs = db.session.query(Export).filter_by(
                    status=ExportStatus.IN_PROGRESS).count()

                # Calculate reuse rate (jobs with reused_from_ref)
                reused_jobs = db.session.query(Export).filter(
                    Export.reused_from_ref.isnot(None)).count()
